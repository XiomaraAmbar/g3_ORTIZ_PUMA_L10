...
public class BTree<E extends Comparable<E>> {
    ...
    public static BTree<Integer> building_Btree(String filename) throws Exception {
        ...
        Map<Integer, BNode<Integer>> mapa = new HashMap<>();
        ...
        while ((linea = reader.readLine()) != null) {
            ...
            nodo.contadorClaves = partes.length - 2;
            for (int i = 0; i < nodo.contadorClaves; i++) {
                nodo.claves.set(i, Integer.parseInt(partes[i + 2].trim()));
            }
            mapa.put(id, nodo);
            niveles.put(id, nivel);
            if (nivel == 0) arbol.raiz = nodo;
        }
        reader.close();

        // Verificación básica de propiedades
        for (Map.Entry<Integer, BNode<Integer>> entrada : mapa.entrySet()) {
            int id = entrada.getKey();
            BNode<Integer> nodo = entrada.getValue();
            if (nodo.contadorClaves > orden - 1 || (nodo != arbol.raiz && nodo.contadorClaves < arbol.minimoClaves)) {
                throw new Exception("ItemNoFound: Nodo " + id + " no cumple con las propiedades del BTree.");
            }
        }
        return arbol;
    }
